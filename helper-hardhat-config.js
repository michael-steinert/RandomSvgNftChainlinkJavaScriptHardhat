...
const autoFundCheck = async (contractAddress, networkName, linkTokenAddress, additionalMessage) => {
    ...
    const contractBalanceHex = await linkTokenContract.balanceOf(contractAddress);
    ...
    if (balance > amount && amount > 0 && contractBalance < amount) {
        /* User has enough LINK to auto-fund and the Contract is not already funded */
        return true;
    } else {
        /* User does not have enough LINK */
        console.log("Account does not have enough LINK to fund Contracts or the Network has the Auto Funding not done by default");
        console.log("npx hardhat fund-link --contract " + contractAddress + " --network " + networkName + additionalMessage);
        return false;
    }
}
